如果说 `docker run` 是手工打造一件兵器，那么 Docker Compose 就是一张全自动化兵工厂的图纸。

在此之前，你启动一个应用可能需要手动运行 N 个命令：启动数据库、启动 Redis、启动后端、启动前端，还得小心翼翼地配置它们之间的网络连接。

Docker Compose 彻底改变了这一切：它允许你通过一个 YAML 文件（`docker-compose.yml`）定义所有的服务，然后用一条命令把它们全部启动起来。

---

## 核心比喻：从"独奏"到"交响乐团"

- Docker (CLI): 就像是独奏家。你告诉它："拉小提琴"，它就开始拉。如果你想听交响乐，你得分别指挥 50 个人，还要告诉他们谁跟谁配合，非常累且容易出错。

- Docker Compose: 就像是乐谱 + 指挥家。
...
2. services (服务): 最核心的部分。
    ...
    - 关键点： 这个名字（`my-database`）会自动变成网络里的域名。你的 Web 应用连接数据库时，Host 直接填 `my-database` 即可，完全不需要知道 IP。
...
结合你之前学的网络 (Networking) 知识，Docker Compose 做了一件极其伟大的事：

它自动为项目创建了一个 Default Network（自定义 Bridge 网络）。

- 在 `docker run` 时代，你需要手动创建网络，手动把容器加进去。

- 在 Docker Compose 时代，只要这几个服务写在同一个 `yml` 文件里，它们天生就在同一个局域网里，并且可以通过服务名直接互相访问。
