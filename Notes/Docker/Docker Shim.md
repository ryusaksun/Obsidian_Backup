这是一个非常棒的深入问题。**Docker Shim (`containerd-shim`)** 是 Docker 架构中一个不起眼但**至关重要**的小角色。

如果说 `runc` 是负责“生孩子”的（创建容器），那么 **Shim 就是负责“带孩子”的（保姆）**。

为了让你彻底理解它，我们需要先明白 Docker 早期架构的一个**痛点**。

---

### 1. 为什么要发明 Shim？（解决“连坐”问题）

在 Docker 的早期版本（v1.11 之前），架构比较简单：

> `Docker Daemon` -> `容器进程`

这意味着 Docker Daemon 是所有容器的“父进程”。这带来了一个巨大的**致命缺陷**：
...
#### **C. 向管理者汇报 (Reaping & Reporting)**

当容器内的进程真正死掉（退出）时：

- `shim` 负责回收僵尸进程（Reaping zombies）。
    
- `shim` 记录容器的**退出状态码**（Exit Code，比如是 0 还是 137）。
...
- **runc (建筑队):** 负责把房子盖好，人住进去，然后**撤场走人**。
    
- **容器 (房主):** 住在房子里的应用。
    
- **Shim (私人管家):**
    
    - **runc** 走之前，雇了一个**私人管家 (Shim)** 站在门口。
        
    - **管家的作用：**
        
        1. **顶雷：** 如果**项目经理 (containerd)** 回家睡觉了，或者换人了，管家依然守在门口，房主（容器）完全不受影响，继续生活。
