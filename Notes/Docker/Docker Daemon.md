...
- **宿主操作系统内核**是公寓的**地基和水电设施**。
    
- **容器**是每一个**独立的房间**。
    
- **Docker Daemon** 就是这栋楼的**物业经理**。
...
- **监听请求 (Listening):** 它一直运行在后台（这也是为什么叫 Daemon，守护进程），<font color="#00b0f0">时刻监听来自 Docker Client 的 API 请求</font>。
    
- **管理对象 (Management):** 它负责<font color="#00b0f0">管理所有的 Docker 对象。你电脑里的 **镜像 (Images)**、**容器 (Containers)**、**网络 (Networks)** 和 **数据卷 (Volumes)**，全都是归它管</font>。它维护着这些东西的状态。
...
1. **Docker Daemon (Server)**
    ...
    - **职责：** 就像我们上一轮说的，它是幕后的**大管家**。它负责创建和管理 Docker 对象（镜像、容器、网络等）。它一直运行在后台。
        
2. **REST API**
    
    - **角色：** 通信协议。
        
    - **职责：** 它是 Client 和 Daemon 之间的**电话线**。它定义了一套标准的指令集，Client 通过这套指令告诉 Daemon 做什么。
...
3. **Docker CLI (Client)**
    ...
    - **职责：** 它是你作为用户直接接触的**界面**。你在终端输入的 `docker run`、`docker ps` 等命令，都是在操作这个 CLI。CLI 负责把你的命令通过 REST API 翻译给 Daemon 听。
...
- **场景：** 假设你在自己的 **笔记本电脑（Mac/Windows）** 上写代码，但你的 Docker 容器需要运行在一台 **远程的 Linux 服务器** 上。
...
- **Docker 引擎** 是**这一整套系统**的名字（Client + API + Daemon）。
    
- **Daemon** 是引擎里**负责干活的那个后台进程**。
    
- 没有 Daemon，引擎就是个空壳（有方向盘但没发动机）；没有引擎这个概念，Daemon 就没法被用户方便地使用。

## Q2：dockerd 是什么？
`dockerd` 其实就是 **Docker Daemon** 的**真名**（程序的文件名）。
...
### 2. `dockerd` 的演变：从“全能王”到“大老板”

这一点非常重要，也是 Docker 技术架构演进的核心。

早期的 Docker 版本中，`dockerd` 是一个**巨型单体**，它干所有的事情：监听请求、管理网络、下载镜像、**甚至亲自去创建容器进程**。

但是现在的 `dockerd` 已经变了。为了稳定性和标准，它把脏活累活都“外包”出去了。现在的 `dockerd` 更像是一个**高级经理**。
...
为了理解 `dockerd` 和它手下（`containerd`, `runc`）的关系，我们可以把 Docker 想象成一家**建筑公司**：
...
- **dockerd (Docker Daemon)**: **公司 CEO / 项目总监**。
    ...
    - 如果 CEO 挂了（crash），整个公司虽然乱套，但现有的工地（容器）可能还能勉强维持一会儿（取决于配置），但接不了新单子了。
...
1. **排查故障（Debug）：** 如果 Docker 启动不了，高手会尝试手动运行 `dockerd` 命令，直接在屏幕上看报错日志，而不是去翻系统日志。
