...
简单来说，`containerd` 是一个工业级的标准容器运行时。

它的地位非常特殊：它曾经是 Docker 的"心腹大将"（一部分），后来被 Docker 公司剥离出来捐赠给了 CNCF（云原生计算基金会），现在它已经成为了整个容器世界的通用底座。
...
4. 地位： 它是现代云原生架构的事实标准。Docker 用它，Kubernetes 也用它。
...
在一个操作系统中，通常只有一个 `containerd` 守护进程在运行，但它可以管理成百上千个容器。
...
3. containerd-shim (物业管家): 这是重点。
    ...
    - 最重要的是： 如果项目经理生病请假了，或者换人了，物业管家依然在坚守岗位，房子里的人（容器）完全感觉不到经理换了。

---

## 总结

- containerd：系统中只有 1 个。它是总管。

- containerd-shim：系统中有 N 个（N = 容器数量）。它是中间人，确保"经理"挂了不影响"干活的"。
