...
- 降采样：由于<font color="#f79646">人眼对亮度敏感而对颜色不敏感</font>，JPEG 允许保留 Y 分量，<font color="#f79646">而对 Cb 和 Cr 分量进行"减半"或更激进的降采样（丢弃部分色度数据）</font>，这是第一层有损压缩。
...
2. 分别编码：
    ...
    - <font color="#f79646">DC 系数（差分编码）</font>：由于相邻块的亮度很接近，使用 DPCM（预测编码），只记录当前块与上一个块 DC 值的差值。

    - <font color="#f79646">AC 系数（行程编码）</font>：对于那一长串的 0，使用 RLE（行程编码），比如记作"连续 15 个 0"。
