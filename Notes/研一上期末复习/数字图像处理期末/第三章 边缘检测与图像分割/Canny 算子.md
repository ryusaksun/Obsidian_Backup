Canny 算子（Canny Edge Detector）被公认为边缘检测领域的"黄金标准"。它由 John F. Canny 在 1986 年提出，不是一个简单的数学公式，而是一个精心设计的多阶段算法。它的核心目标是：找到尽可能多的真实边缘，同时尽可能少地误报噪声，并且边缘定位要极其精准。
...
这一步是 Canny 算子的精华所在，目的是把边缘变细（瘦身）。

- 原理：对于每一个被认为是边缘的像素，去看看它在梯度方向上的两个邻居（前一个和后一个像素）。
...
## 第四步：双阈值检测（Double Thresholding）

经过上一步，虽然边缘细了，但可能还有很多微弱的假边缘（由噪声引起）。Canny 设定了两个阈值：高阈值（High）和低阈值（Low）。

- 强边缘：梯度值 > 高阈值。这些肯定是真边缘，直接保留（标记为强）。
- 弱边缘：低阈值 < 梯度值 < 高阈值。这些可能是真边缘，也可能是噪声，待定（标记为弱）。
- 非边缘：梯度值 < 低阈值。这些肯定是噪声，直接扔掉（置为 0）。

## 第五步：滞后边界跟踪（Edge Tracking by Hysteresis）
