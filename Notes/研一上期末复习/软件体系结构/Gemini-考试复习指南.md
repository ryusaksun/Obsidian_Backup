...
架构风格是一组设计决策的集合，它定义了一类架构的词汇表（构件和连接件类型）以及它们组合的约束条件。而模式通常指针对特定上下文中的常见问题的解决方案。考试中，不仅要区分风格与模式，还要掌握各种风格的优缺点及应用场景。
...
- Controller（控制器）：处理用户输入（键盘、鼠标），将其转化为对 Model 的更新操作或对 View 的改变。
