<font color="#00b0f0">`less` 是一款比 `more` 更强大的文件阅读工具</font>。它的名字来源于"Less is more"（少即是多），但功能上它其实是 `more` 的"超级增强版"。

它最大的特点是支持随意上下翻页，而且在打开大文件时不会一次性加载整个文件（速度极快），非常适合查看几百兆甚至几个 G 的日志文件。
